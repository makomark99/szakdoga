...
<form class="row g-3 " action="includes/addplayer.inc.php" method="post">
	...
	<div class="col-md-4 col-lg-2">
		...
		<input name="pMCD" type="text" title="Csak betűk használata lehetséges" pattern="^[a-zA-Zéáűőúöüóí\s]*$"
			class="form-control" id="mc" placeholder="Név / helyszín">
	</div>
  ...
  ?>
	...
	<div class="col-md-4 col-lg-2 ">
		...
		<input name="pSsn" type="number" pattern='[0-9]{9}' class="form-control" id="ssn" placeholder="Tajszám">
	</div>

	<div class="col-md-4 col-lg-2 ">
		<label for="pHA" class="form-label">Lakhely (település)</label>
		<input name="pHA" placeholder="Lakhely"
			title="Csak betűk, számok, ',' '.' ';' és '/' jelek használata lehetséges"
			pattern="^[a-zA-Z0-9éáűőúöüóíÁÉÍŰÚŐÖÜÓ\s\/\,\.\;]*$" type="text" class="form-control" id="pHA">
	</div>
    ...
    if (isset($_GET["error"])) {
        ...
        if ($_GET["error"]== "invalidpbdate") {
            errorAlert("Érvénytelen születési dátum", "manage_users.php", true);
        }
        if ($_GET["error"]== "invalidpLMCDate") {
            errorAlert("Sportorvosi vizsgálathoz kizárólag múltbeli időpont adható meg!", "p_add.php", true);
        }
        ...
        if ($_GET["error"]== "invalidssn") {
            errorAlert("Érvénytelen tajszámot adott meg!", "p_add.php", true);
        }
        if ($_GET["error"]== "none") {
